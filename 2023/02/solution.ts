        ...
        for (let count of group.split(","))
            for (let color of colors())
                if (count.includes(color))
                    (game.colors[color] ||= []).push(Number(count.replace(/\D/g, "")));
})

console.log(sum(games.filter(game => colors().every(color => Math.max(...game.colors[color]) <= maxes[color])).map(game => game.id)));
console.log(sum(games.map(game => product(colors().map(color => Math.max(...game.colors[color]))))));